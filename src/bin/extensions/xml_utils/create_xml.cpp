...
int create_default_config_xml(const char *filename) {
    ...
    //Create child nodes directly in the root node
    addConfigNode((xmlNodePtr)(config_root), "port", "9007");
    addConfigNode((xmlNodePtr)config_root, "db_name", "lightdb");
    addConfigNode((xmlNodePtr)config_root, "db_user", "root");
    addConfigNode((xmlNodePtr)config_root, "db_passwd", "root");

    //Dump an XML document to a file
    char __saved_xml_fname[128] = {0};

    //check extension
    auto *__ext = filename + strlen(filename) - 4;
