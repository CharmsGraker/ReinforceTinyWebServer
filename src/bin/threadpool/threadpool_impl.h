...
private:
    ...
    int doSchedule(T &task) {
        try {
            ...
            __scheduler->schedule(std::move(task));
...
private:
    ...
    threadsafe_queue<T> m_workQueue; //请求队列
...
bool threadPool<T>::append(T &&task, int state) {
    task->m_state = state;
    m_workQueue.push(std::forward<T>(task));
    return true;
}
