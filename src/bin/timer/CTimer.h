...
#ifndef GRAKER_CTimer_H
#define GRAKER_CTimer_H
...
class CTimer {
public:
    explicit CTimer(const std::string &sTimerName = "");   //构造定时器，附带名称
    ~CTimer();
    ...
    bool SyncOnce(int msTime, callable &&fun, arguments &&... args) {
        std::function<typename std::result_of<callable(arguments...)>::type()>
                task(std::bind(std::forward<callable>(fun), std::forward<arguments>(args)...)); //绑定任务函数或lambda成function
        return Start(msTime, task, false, false);
    }

    template<typename callable, typename... arguments>
    bool SyncLoop(int msTime, callable &&fun, arguments &&... args) {
        std::function<typename std::result_of<callable(arguments...)>::type()>
                task(std::bind(std::forward<callable>(fun), std::forward<arguments>(args)...)); //绑定任务函数或lambda成function
        return Start(msTime, task, true, false);
    }
    ...
    bool AsyncOnce(int msTime, callable &&fun, arguments &&... args) {
        std::function<typename std::result_of<callable(arguments...)>::type>
                task(std::bind(std::forward<callable>(fun), std::forward<arguments>(args)...));
    ...
    /**
     ...
     @param msTime 延迟及间隔时间（毫秒）
     @param fun 函数接口或lambda代码块,for member function, it has a explict args which need bind to the object ptr
     @param args 参数
     @return true:已准备执行，否则失败
     */
    template<typename callable, typename... arguments>
